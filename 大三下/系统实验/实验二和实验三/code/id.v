...
// Module:  id
// File:    id.v
// Description: 译码阶段——对指令进行译码，产生执行阶段需要的控制信号和操作数
// 该模块是OpenMIPS五级流水线的第二阶段（译码阶段），主要完成：
//   (1) 根据op/op2/op3/op4字段译出指令类型，设置相应的控制信号
//   (2) 处理数据冒险（load相关），必要时产生流水线暂停请求
//   (3) 处理分支/跳转指令，计算目标地址和延迟槽标记
//   (4) 检测异常指令（syscall、eret、无效指令）
//////////////////////////////////////////////////////////////////////
...
module id(
  ...
  //assign excepttye_is_trapinst = 1'b0;
	...
	always @ (*) begin	
		...
		// ==== R-type数据移动指令 (MFHI, MFLO, MTHI, MTLO) ====
								`EXE_MFHI: begin
		...
		// ==== R-type算术运算指令 (ADD, ADDU, SUB, SUBU) ====
								`EXE_ADD: begin
		...
		// ==== R-type乘除法指令 (MULT, MULTU, DIV, DIVU) — 结果存HI/LO ====
								`EXE_MULT: begin
		...
		// ==== R-type跳转指令 (JR, JALR) ====
					...
					endcase	
          case (op3)
		// ---- Trap类指令 (TEQ/TGE/TGEU/TLT/TLTU/TNE/SYSCALL) ----
								`EXE_TEQ: begin
	...
	// ---- Load指令 (LB/LBU/LH/LHU/LW/LL/LWL/LWR) — 从内存读取 ----
				`EXE_LB:			begin
	...
	// ---- Store指令 (SB/SH/SW/SWL/SWR/SC) — 写入内存 ----
				`EXE_SB:			begin
	...
	// ---- REGIMM类指令 (BGEZ/BGEZAL/BLTZ/BLTZAL/TEQI/TGEI/...) ----
				`EXE_REGIMM_INST:		begin
					case (op4)
		// ---- 条件分支 (BGEZ, BGEZAL, BLTZ, BLTZAL) ----
						`EXE_BGEZ:	begin
		...
		// ---- Trap立即数 (TEQI/TGEI/TGEIU/TLTI/TLTIU/TNEI) ----
						`EXE_TEQI:			begin
	...
	// ---- SPECIAL2类 (CLZ/CLO/MUL/MADD/MADDU/MSUB/MSUBU) ----
	...
	// ---- CP0指令和ERET (MFC0/MTC0/ERET) ----
		...
		// ===========================================================================
		// reg1_o生成逻辑：Load数据相关处理 + 数据前推
		// 优先级：load相关暂停 > ex前推 > mem前推 > Regfile读 > imm
		// ===========================================================================
	always @ (*) begin
