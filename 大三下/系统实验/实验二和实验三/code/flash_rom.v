...
//////////////////////////////////////////////////////////////////////
// Module:  flash_rom
// Description: SPI Flash ROM 控制器 — Wishbone 从设备接口
//             两大功能:
//               1. SPI Flash 控制 — 通过 SPI 接口读写板载 Flash 存储器
//               2. Wishbone 总线从设备 — 响应 CPU 取指/数据访问
//             Flash 作为 CPU 的指令存储器 (inst_rom)
//             内部状态机: IDLE → START → INST_OUT → ADDR1/2/3_OUT
//                         → WRITE_DATA 或 READ_DATA(1~5) → WAITING → ENDING
//             支持连续读模式 (flash_continue 信号)
//             注: 复位后需等待 Flash 初始化完成才能开始取指
//////////////////////////////////////////////////////////////////////

// main two part: 1.spi flash control 2. wb main bus
module flash_rom(
    input wire wb_clk_i, // Wishbone���ߵ�ʱ���ź�
    input wire wb_rst_i, // ��λ�źţ����ڳ�ʼ��������ģ��״̬
    input wire wb_cyc_i, // ��ʾһ��Wishbone�������ڵĿ�ʼ�ͽ���
    input wire wb_stb_i, // ѡͨ�źţ�����Wishbone���豸������һ����Ч�Ĵ���
    input wire wb_we_i, // дʹ���źţ����ֶ�д����
    input wire [3:0] wb_sel_i, // �ֽ�ѡ���źţ�����ѡ��32λ�����е���Ч�ֽ�
    input wire [23:0] wb_adr_i, // ��ַ�źţ�ָ������flash�洢�ĵ�ַ
    input wire [31:0] wb_dat_i, // д�����źţ���Wishbone���ߴ��䵽flash�洢������
    output reg [31:0] wb_dat_o, // �������źţ���flash���䵽Wishbone���ߵ�����
    output reg        wb_ack_o, // Ӧ���źţ���ʾ���ݴ������
    
    input wire flash_continue, // �ⲿ�źţ�����ģ��ĳ�������
    
    output reg cs_n, // Ƭѡ�źţ�����flash�洢�豸��ʹ��״̬
    input  sdi, // ������������
    output reg sdo, // �����������
    output reg wp_n, // д�����ź�
    output reg hld_n // �����ź�
...
(* dont_touch = "true", mark_debug = "true" *)reg         sck;
(* dont_touch = "true", mark_debug = "true" *)reg  [4:0]  state;
reg  [4:0]  next_state;

(* dont_touch = "true" *)reg  [7:0]   instruction;
(* dont_touch = "true", mark_debug = "true" *)reg  [7:0]   datain_shift;
...
reg         temp;
(* dont_touch = "true", mark_debug = "true" *)reg  [3:0]  sdo_count;
reg  [15:0] page_count;
reg  [7:0]  wait_count;
...
(* dont_touch = "true", mark_debug = "true" *)reg [31:0] read_data;

// ILA probes: sdi/sdo/cs_n are ports, expose as marked nets so Set Up Debug lists them
(* dont_touch = "true", mark_debug = "true" *) wire sdi_dbg;
(* dont_touch = "true", mark_debug = "true" *) wire sdo_dbg;
(* dont_touch = "true", mark_debug = "true" *) wire cs_n_dbg;
assign sdi_dbg  = sdi;
assign sdo_dbg  = sdo;
assign cs_n_dbg = cs_n;
